        ...
        this.bot.r.table('users').get(user.id).run((error) => {
          ...
          if (args[1].toLowerCase() === 'developer') {
            ...
            this.bot.r.table('users').get(user.id).update({
              ...
              if (error) return this.bot.utils.handleDatabaseError(error);
              return ctx.send(`${this.bot.constants.emojis.SUCCESS} | User **${user.username}#${user.discriminator}** lost the **Donator** badge.`);
            });
            ...
            this.bot.r.table('users').get(user.id).update({
              ...
              if (error) return this.bot.utils.handleDatabaseError(error);
              return ctx.send(`${this.bot.constants.emojis.SUCCESS} | User **${user.username}#${user.discriminator}** lost the **Bot Owner** badge.`);
            });
          }
        });
