...
module.exports = class ShopCommand extends Command {
  ...
  async execute(ctx, args) {
    if (!args[0]) {
      ...
      if (args[1].toLowerCase() === 'ring') {
        ...
        this.bot.r.table('users').get(ctx.author.id).run((error, u) => {
          this.bot.r.table('users').get(ctx.author.id).update({
            ...
            if (error) return this.bot.utils.handleDatabaseError(error);
            ctx.send(":ok_hand: | You have bought 1 pickaxe for 20,000¥");
          });
        });
      }
      ...
      if (args[1].toLowerCase() === 'ring') {
        if (this.bot.r.table('users').get(ctx.author.id).run().inventory.pickaxe < 1) return ctx.send(`${this.bot.constants.emojis.ERROR} | You don't have the 'pickaxe' item.`);
        ...
        this.bot.r.table('users').get(ctx.author.id).run((error, u) => {
          this.bot.r.table('users').get(ctx.author.id).update({
            ...
            if (error) return this.bot.utils.handleDatabaseError(error);
            ctx.send(":ok_hand: | You have selled 1 pickaxe for 4,000¥");
          });
        });
      } else if (args[1].toLowerCase() === 'ring') {
        if (this.bot.r.table('users').get(ctx.author.id).run().inventory.ring < 1) return ctx.send(`${this.bot.constants.emojis.ERROR} | You don't have the 'ring' item.`);
