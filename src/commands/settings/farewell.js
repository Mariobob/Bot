...
module.exports = class FarewellCommand extends Command {
  constructor(bot) {
    super(bot, {
      command: 'farewell',
      description: 'This is where you can set your goodbyes to people.',
      syntax: 'farewell ["set"/"list"/"keys"/"variables"] ["channel"/"message"/"enable"] [value]',
      category: 'Settings',
      guildOnly: true
    });
  ...
  async execute(ctx, args) {
    ...
    if (args[0].toLowerCase() === 'set') {
      if (!args[1]) return ctx.send(`${this.bot.constants.emojis.ERROR} | You must provide a key. View \`${ctx.prefix}farewell keys\` to see what the current keys are.`);
      if (!args[2]) return ctx.send(`${this.bot.constants.emojis.ERROR} | You must provide a value. View \`${ctx.prefix}farewell variables\` to see what the current variables are.`);

      if (args[1].toLowerCase() === 'channel') {
        this.bot.utils.resolveChannel(args[2], ctx.guild)
          .then(async(c) => {
            ctx.send(`${this.bot.constants.emojis.MEMO} | Are you sure that you want to receive farewell messages in <#${c.id}>?\nType __y__es or __n__o. You have 60 seconds to decide.`);
            ...
            if (['no', 'n'].includes(message.content.toLowerCase())) {
              return ctx.send(`${this.bot.constants.emojis.ERROR} | You will not receive farewell messages in <#${c.id}>.`);
            } else {
              this.bot.r.table('guilds').get(ctx.guild.id).update({
                ...
                return ctx.send(`${this.bot.constants.emojis.SUCCESS} | You will receive farewell messages in <#${c.id}>.`);
              });
            }
          });
        ...
        ctx.send(`${this.bot.constants.emojis.MEMO} | Are you sure you wanna ${enabled === true ? "enable" : "disable"} farewell messages?\nType __y__es or __n__o. You have 60 seconds to decide.`);
        const message = await this.bot.collector.awaitMessage(ctx.channel.id, ctx.author.id, 60e3, this.filter);

        if (!message) return ctx.send(`${this.bot.constants.emojis.ERROR} | Prompt timed out.`);

        if (['no', 'n'].includes(message.content.toLowerCase())) {
          return ctx.send(`${this.bot.constants.emojis.ERROR} | You will not ${enabled === true ? "enable" : "disable"} farewell messages.`);
        } else {
          this.bot.r.table('guilds').get(ctx.guild.id).update({
            farewell: {
              enabled
            ...
            if (error) return this.bot.utils.handleDatabaseError(error);
            return ctx.send(`${this.bot.constants.emojis.SUCCESS} | You have ${enabled === true ? "enabled" : "disabled"} farewell messages.`);
          });
        }
      } else if (args[1].toLowerCase() === 'message') {
        ctx.send(`${this.bot.constants.emojis.MEMO} | Are you sure you wanna set \`${args.slice(2).join(" ")}\` as your farewell message?\nType __y__es or __n__o. You have 60 seconds to decide.`);
        const message = await this.bot.collector.awaitMessage(ctx.channel.id, ctx.author.id, 60e3, this.filter);
      ...
      this.bot.r.table('guilds').get(ctx.guild.id).run((error, gConfig) => {
        if (error) return this.bot.utils.handleDatabaseError(error);
        return ctx.sendCode('md', `# Current "Farewell" Settings for ${ctx.guild.name}:\nEnabled: ${gConfig.farewell.enabled}\nMessage: ${gConfig.farewell.message}\nChannel (ID): ${gConfig.farewell.channel}`);
      });
    } else if (args[0].toLowerCase() === 'keys') {
      return ctx.sendCode('md', `# Current Keys:\n* set -> Sets the key/value.\n* list -> Sees the current farewell settings.\n* variables -> Shows the variables for the farewell message.`);
